...
Nebula already ships a FastAPI gateway, a Next.js operator console, PostgreSQL-backed governance, Qdrant-backed semantic cache, OpenAI-like chat completions, streaming support, tenant and API-key management, policy management, usage ledger recording, runtime health, and an optional hosted control plane with a metadata-only default export contract. S01, S02, S03, and S04 of M001 are now complete: the public `POST /v1/chat/completions` adoption boundary is explicit and test-backed in `docs/adoption-api-contract.md`; the supported self-hosted quickstart and operating-model references live in `docs/quickstart.md` and `docs/production-model.md`; the repo now includes a canonical migration proof in `docs/reference-migration.md` plus executable request-to-ledger correlation coverage in `tests/test_reference_migration.py`; and the canonical day-1 value walkthrough now lives in `docs/day-1-value.md` with aligned Playground, ledger, and Observability surfaces that make route, provider, fallback, policy, and dependency-health evidence legible immediately after adoption. The remaining M001 gap is S05: exercising the joined-up adoption story end to end so docs, migration proof, public headers, persisted evidence, and operator-visible surfaces all align in one final integrated walkthrough.
