...
- [x] M004: Hosted Adoption Reinforcement — Completed milestone close-out and validation for the hosted/control-plane reinforcement track. The assembled work now gives operators a clearer hosted adoption story without widening hosted authority: S01 centralized the canonical hosted trust-boundary vocabulary and bounded-action guardrails in `console/src/lib/hosted-contract.ts`; S02 added a real hosted fleet-posture reading layer on the deployments page through shared posture derivation, a top-of-page fleet summary, scan-time row cues for pending/linked/stale/offline/revoked/unlinked states, and aligned bounded-action availability semantics; S03 assembled the canonical hosted integrated proof doc plus focused Vitest and Playwright walkthrough coverage proving that the public trust-boundary page, deployments entrypoint, mixed-fleet interpretation, detail drawer, and bounded credential-rotation framing reinforce confidence without shifting serving-time authority away from the local runtime; and S04 closed the remaining wording/evidence-mapping gaps, fixed the standalone Next.js Playwright production-server path, and reran the full verification bar successfully. Milestone close-out also verified non-`.gsd/` implementation changes against `codex/v3-api-adoption`, confirmed all success criteria and definition-of-done checks, and validated R014 plus R032–R038.
