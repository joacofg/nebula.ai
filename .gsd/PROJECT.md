...
M007 has S01 through S04 complete. The console still follows the explicit operator-surface role contract from S01, S02 tightened the supporting seams underneath it, S03 reworked Observability into a selected-request-first investigation flow, and S04 has now reworked policy preview into a compare-before-save decision surface. Operators can load a tenant policy, compare the current baseline against a candidate draft using recent persisted traffic, see an explicit preview-only decision summary before bounded changed-request evidence, and decide whether to keep iterating or save without the page drifting into analytics or dashboard framing. The policy route entrypoint preserves the existing split between preview and save mutations, keeps the simulate payload bounded (`limit: 50`, `changed_sample_limit: 5`), and clears stale preview evidence on tenant switch and save success. Fresh focused Vitest coverage guards decision-first preview hierarchy, null and rollout-disabled routing parity rendering, explicit non-saving wording, bounded payload shape, and reset semantics before the integrated S05 close-out work.
