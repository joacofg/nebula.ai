...
M002 is now in progress with S01 complete in this worktree. The highest-impact operator-facing console surfaces now align to the runtime-truth tenant/API-key model: API Keys explains tenant inference versus explicit `X-Nebula-Tenant-ID`, Playground is framed as an admin-only non-streaming corroboration surface, and Observability is framed as persisted request evidence plus dependency-health context. S02 and S03 remain to prove app/workload guidance without fake runtime entities and then assemble the integrated production-structuring walkthrough.
