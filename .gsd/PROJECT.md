...
M007 has started with S01 complete. The console now has an explicit operator-surface role contract: Observability is structurally request-investigation-first, the selected request evidence section leads the page after filters, request detail states that the persisted ledger row is the authoritative evidence record, and policy preview is locked as a replay-before-save comparison seam rather than a blended analytics/editor surface. Focused console coverage now guards section order, subordinate framing, and negative boundaries against dashboard, routing-studio, and analytics-product drift across these surfaces.
